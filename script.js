...
function displayDetails() {
	...
	document.querySelector("#price").innerText =
		parseFloat(savings).toLocaleString("en-US", opts) + "%";

	pie.data.datasets[0].data[0] = P;
	pie.data.datasets[0].data[1] = payabaleInterest/100;
	pie.update();
	line.update();
}
...
function initialize() {
	...
	pie = new Chart(document.getElementById("pieChart"), {
		type: "doughnut",
		data: {
			labels: ["CO2 Emissions", "Carbon Footprint Reduction"],
			datasets: [
				{
					label: "Home Loan Details",
					data: [0, 0],
					backgroundColor: ["rgb(179, 179, 179)", "rgb(140, 255, 153)"],
					hoverOffset: 4
				}
			]
		},
		options: {
			plugins: {
				title: {
					display: true,
					text: "Projected Carbon Footprint Emissions"
				}
			}
		}
	});
